...
export default class EditAssessmentSubmission extends Component {
            ...
            if (this.state.date_modified === false && this.state.file_modified === false) {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.put(BASE_URL + 'submissions/' + this.props.match.params.id, currentAssignment)
                            .then(() => {
                                Swal.fire({
                                    title: 'Assessment Submissions Successfully Updated',
                                    type: 'success',
                                    confirmButtonText: 'OK',

                                }).then((result) => {
                                    if (result.value) {
                                        this.props.history.push('/submissions/' + this.props.match.params.id);
                                    }
                                });
                    ...
                    axios.post(BASE_URL + 'submissions/upload-file', fd, {
                        ...
                        .then(res => {
                            if (res.data.status !== 400) {
                                ...
                                axios.post(BASE_URL + 'submissions/delete-file', deleteFile);
    ...
    render() {
        return (
            ...
            <h5 className="mt-2">Edit Submissions Details</h5>
        </div>
        <br/>
